...
- **구조**:
    ...
    - **`grammar_map` (문법 지도)**: `grammar.csv` 기반. 조사, 어미뿐만 아니라 '문법적 표현'(`어지다` 등)도 포함.
    - **`idiom_map` (관용구 지도)**: `grammar.csv` 기반.
        - **생성 원리**: 문법 데이터 중 '대표형'에 **띄어쓰기**가 포함되어 있거나, 분류가 **'표현'**으로 지정된 항목을 추출합니다.
        - **구조**: `(시작 단어) -> [나머지 단어 시퀀스]` 형태로 변환하여, 문장에서 연쇄적으로 단어가 등장할 때 이를 하나의 관용구로 인식하도록 합니다.
        - **예시**: `-ㄹ 수 있다`, `-기로 하다` 등.
...
#### 3-1. 관용구(Expression) 패턴 매칭 [최우선]
- **논리**: 현재 토큰이 `idiom_map`의 시작 단어와 일치하는지 확인합니다.
- **동작**: 뒤따르는 토큰들이 관용구의 구성 요소와 정확히 일치하면, 해당 토큰들을 모두 하나로 묶습니다.
